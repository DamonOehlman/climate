...
CleaverRepl.prototype.getHelp = function(command) {
    ...
    if (realCommand && this.help[realCommand]) {
        this.cleaver.out(this.help[realCommand] + '\n');
    }
    else if (realCommand) {
        this.cleaver.out('No help found for command: ' + command + '\n');
    }
    // otherwise, if no command was specified, then output the list of commands
    else {
        this.cleaver.out('TODO: list the commands');
    }
};
...
module.exports = function(cleaver, prompt, opts) {
    ...
    repl.command('help', function() {
        repl.getHelp.apply(repl, Array.prototype.slice.call(arguments));
    });
