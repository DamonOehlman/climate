...
function _configureStreams(cleaver, input, output) {
    ...
    if (cleaver.input.isTTY) {
        ...
        cleaver.rli = rl.createInterface(cleaver.input, output || process.stdout);
        // tty.setRawMode(true);
        // cleaver.input._handle.setRawMode(true);
        
        cleaver.rli.on('line', processLine);
...
Cleaver.prototype.prompt = function(text, maskChar) {
    ...
    return this._queue('prompt', function() {
        ...
        if (cleaver.rli) {
            debug('writing: ' + text);
            cleaver.rli.question(text + ' ', function(line) {
                cleaver._queueLine(line);
            });
        }
    });
};
